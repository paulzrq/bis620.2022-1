#' process the data
#' This function builds a linear model with a dependent variable death day "DTHTY"
...
test_DTHDY <- function(x){
  ...
  m1 <- lme(y ~ x1_sex+x2_race+x3_atrt+x5_age+x6_weight+x7_height, random=~1|x4_biomark, data=unique(dataset),na.action=na.exclude)
  print(summary(m1))
