...
#' "FOLFOX alone" and "Panitumumab + FOLFOX" and see different treatments with different age group and visualize it.
...
survivalATRT_age <- function(dl,detail=FALSE) {
  ...
  summary_table <- comb |>
    ...
    tbl_summary(by = "ATRT")
  age_group <- c(1:length(comb$AGE))
  for (i in 1:length(comb$AGE)) {
